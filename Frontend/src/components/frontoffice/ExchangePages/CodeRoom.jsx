...
const CodeRoom = () => {
  const { roomId } = useParams(); // ✅ récupérer roomId depuis URL
  ...
  useEffect(() => {
    socket.emit("join-room", roomId); // ✅ rejoindre la room spécifique
    ...
    return () => {
      ...
      socket.emit("leave-room", roomId); // optionnel pour quitter proprement
    };
  }, [roomId]);

  const handleCodeChange = (newCode) => {
    setCode(newCode);
    socket.emit("code-change", { roomId, code: newCode }); // ✅ envoyer code + roomId
  };
